  ...
  useEffect(() => {
    ...
    worker.onmessage = (e) => {
      ...
      if (type === "status") {
        ...
        // Worker now sends a transferable ArrayBuffer (zero-copy) of the
        // 1024x1024 sigmoid probability map. Reconstruct the typed view.
        const { mask: maskBuffer, width, height, score: s } = data;
        const mask = new Float32Array(maskBuffer as ArrayBuffer);
        ...
        if (octx && bctx && rctx) {
          ...
          // Pass 2: soft border via Chebyshev distance transform, but only
          // inside (mask bbox + BORDER_RADIUS). Cuts the work from 1M
          // pixels per frame to roughly the perimeter band of the mask.
          const borderData = new ImageData(width, height);
          if (maxX >= 0) {
            const x0 = Math.max(0, minX - BORDER_RADIUS);
            const y0 = Math.max(0, minY - BORDER_RADIUS);
            const x1 = Math.min(width  - 1, maxX + BORDER_RADIUS);
            const y1 = Math.min(height - 1, maxY + BORDER_RADIUS);
            for (let y = y0; y <= y1; y++) {
              const row = y * width;
              for (let x = x0; x <= x1; x++) {
                const i = row + x;
                ...
                let best = -1;
                for (let dy = -BORDER_RADIUS; dy <= BORDER_RADIUS; dy++) {
                  const ny = y + dy;
                  ...
                  if (best === 1) break; // can't get closer than 1
                }
                if (best >= 0) {
                  const idx = i * 4;
